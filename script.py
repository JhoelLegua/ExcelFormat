...
def procesar_archivos_en_memoria(archivos_excel):
    """
    Procesa una lista de archivos Excel en memoria y devuelve el archivo unificado como BytesIO
    
    Args:
        archivos_excel: Lista de objetos archivo (FileStorage de Flask o rutas de archivos)
    
    Returns:
        BytesIO: Archivo Excel procesado en memoria
    """

    print("Iniciando unificacion estricta...")
    
    # Ordenar archivos si tienen nombre
    if hasattr(archivos_excel[0], 'filename'):
        archivos_excel.sort(key=lambda x: x.filename if x.filename else '')
    ...
    for i, archivo in enumerate(archivos_excel):
        ...
        if hasattr(archivo, 'filename'):
            nombre = archivo.filename or f"archivo_{i+1}"
            # Leer archivo desde Flask FileStorage
            archivo_stream = BytesIO(archivo.read())
        else:
            # Si es una ruta de archivo (para compatibilidad con uso directo)
        ...
        try:
            ...
            df = pd.read_excel(archivo_stream, header=2, dtype=object)
    ...
    # ---------------------------------------------------------
    # GENERACIÓN DEL ARCHIVO FINAL EN MEMORIA
    # ---------------------------------------------------------
    if lista_dataframes:
        ...
        df_final.to_excel(output_stream, index=False)
    
        # Formatear la primera fila (headers)
        print("Aplicando formato a headers...")
        output_stream.seek(0)  # Volver al inicio
        ...
        # Guardar el archivo formateado en memoria
        final_output = BytesIO()
        wb.save(final_output)
        final_output.seek(0)  # Volver al inicio para lectura
        
        print(f"\nListo! Archivo generado en memoria")
...
# Mantener compatibilidad con ejecución directa (para pruebas locales)
if __name__ == "__main__":
    # Configuración original para ejecución directa
    ...
    if archivos:
        archivos.sort()
        resultado = procesar_archivos_en_memoria(archivos)
        
        if resultado:
            # Guardar el resultado en disco para ejecución directa
